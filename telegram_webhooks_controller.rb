class TelegramWebhooksController < ApplicationController
  ...
  def receive
    ...
    if message
      ...
      # Check if the user is currently submitting a task
      if tg_user.current_task_id.present?
        handle_submission_response(tg_user, text, message, reply_to_message_id) # Handle the user's response for submission
      else
        case text
        when '/tasks'
          list_tasks(tg_user)
